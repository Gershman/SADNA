...
namespace ConsoleApplication1
{
    static class ParserUtils
    {
        ...
        public static string GetSubStrThatStartsWithStr(string srcStr ,string startStr,bool withMaxLength, int maxLength, bool includeStartStr)
        {
            int startIndexStr;
            string result;
            if (includeStartStr)
            {
                startIndexStr = srcStr.IndexOf(startStr);
            }
            else // don't include
            {
                startIndexStr = srcStr.IndexOf(startStr) + startStr.Length;
            }

            if (withMaxLength)
            {
                result = srcStr.Substring(startIndexStr, maxLength);
            }
            else
            {
                result = srcStr.Substring(startIndexStr);
            }
